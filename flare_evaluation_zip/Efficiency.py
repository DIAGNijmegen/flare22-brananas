...
def save_result(
    start_time, sleep_time, json_path, gpu_list_, cpu_list_copy, RAM_list_copy
):
...
if __name__ == "__main__":
    ...
    try:
        ...
        # )
        cmd = 'docker container run --gpus="device=0" --name {} --rm -v $PWD/inputs/:/workspace/inputs/ -v $PWD/outputs/:/workspace/outputs/ {}:latest /bin/bash -c "sh predict.sh" '.format(
        ...
        save_result(
            t0, args.sleep_time, json_path, gpu_list_copy, cpu_list_copy, RAM_list_copy
        )
